...
# [model2 방식]
   ...
   - forward(request객체 유지)
      	...
      	단순 조회를 요청할 경우 사용된다.
      	목록, 검색 등
      	request소코프의 데이터를 기억한다.(화면에 전달할 데이터가 있을 때)
	redirect : request를 초기화 시킨다.(화면에 전달할 데이터가 없을 때)
	컨트롤러에서는 forword인지 redirect 를 불린으로 프론트에 보내고 프론트에서
	true,false로 구분하여 c.jsp로 보내준다.
